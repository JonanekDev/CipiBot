import { APIGuild, PermissionFlagsBits, RESTGetAPICurrentUserGuildsResult, RESTGetAPICurrentUserResult, RESTPostOAuth2AccessTokenResult } from "discord-api-types/v10";
...
export class AuthService {
    ...
    async requestDiscordGuilds(accessToken: string, userId: string): Promise<RESTGetAPICurrentUserGuildsResult> {
        ...
        const res = await axios.get<RESTGetAPICurrentUserGuildsResult>('https://discord.com/api/users/@me/guilds', {
            headers: {
                Authorization: `Bearer ${accessToken}`,
            },
        });
